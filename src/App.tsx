...
import { EyeIcon, EyeOffIcon, LockIcon, LogoMark, ShieldIcon } from './components/Icons'
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-ivory text-ink">
      ...
      <header className="relative mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-5 sm:px-8 lg:px-10">
        <a className="flex items-center gap-3 rounded-xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand" href="#top" aria-label="GLICOGIG, torna all’inizio">
          <LogoMark />
          <div><p className="font-display text-xl font-bold tracking-tight text-brand">GLICOGIG</p><p className="text-[10px] font-extrabold tracking-[0.2em] text-muted uppercase">Food signal</p></div>
        </a>
        <div className="inline-flex items-center gap-2 rounded-full border border-brand/10 bg-white/75 px-3 py-2 text-xs font-bold text-brand shadow-sm backdrop-blur"><ShieldIcon className="size-4 text-mint" /><span className="hidden sm:inline">Accesso personale</span><span className="sm:hidden">Privato</span></div>
      </header>
      <div id="top" className="relative mx-auto max-w-7xl px-5 pb-12 sm:px-8 lg:px-10 lg:pb-20">
        ...
        <section className="mb-5 rounded-3xl border border-brand/10 bg-white/75 p-4 shadow-sm backdrop-blur sm:flex sm:items-center sm:gap-4 sm:p-5">
          <div className="flex min-w-0 flex-1 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-brand-soft text-brand"><LockIcon className="size-5" /></span><div className="min-w-0 flex-1"><label className="text-xs font-extrabold tracking-wide text-brand uppercase" htmlFor="access-key">Chiave di accesso personale</label><div className="relative mt-1.5"><input id="access-key" className="h-11 w-full rounded-xl border border-line bg-white px-3 pr-11 text-sm text-ink outline-none transition placeholder:text-muted/60 focus:border-brand focus:ring-3 focus:ring-brand/10" type={showAccessKey ? 'text' : 'password'} value={accessKey} onChange={(event) => setAccessKey(event.target.value)} placeholder="Richiesta sul deploy Vercel" autoComplete="current-password" /><button className="absolute inset-y-0 right-0 grid w-11 place-items-center text-muted transition hover:text-brand" type="button" onClick={() => setShowAccessKey((visible) => !visible)} aria-label={showAccessKey ? 'Nascondi chiave' : 'Mostra chiave'}>{showAccessKey ? <EyeOffIcon className="size-4" /> : <EyeIcon className="size-4" />}</button></div></div></div>
          <p className="mt-3 max-w-sm text-xs leading-5 text-muted sm:mt-0">Resta nella sessione del browser e viene verificata soltanto dalla Function privata.</p>
        </section>
        <div className="grid items-start gap-5 lg:grid-cols-[0.88fr_1.12fr]"><PhotoCard image={image} busy={busy} onFile={handleFile} onAnalyze={() => void handleAnalyze()} onClear={clearImage} /><ResultPanel status={status} result={result} error={error} hasImage={Boolean(image)} onRetry={() => void handleAnalyze()} /></div>
